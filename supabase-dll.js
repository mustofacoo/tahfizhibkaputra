        ...
        const app = createApp({
            setup() {
                ...
                const SUPABASE_CONFIG = {
                    url: 'https://sqxsuejtomqyqnyihsjq.supabase.co',
                    key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNxeHN1ZWp0b21xeXFueWloc2pxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTI4NzM2NzEsImV4cCI6MjA2ODQ0OTY3MX0.5LWfwWk-40lR1gsU0c6InP_NgGg3scMSbsEg7wcb3OY'
                };
                ...
                const getSantriProgressPage = (santriId) => {
                    ...
                    // Ambil semua tanggal dan urutkan dari terbaru ke terlama
                    const dates = Object.keys(progress).sort((a, b) => b.localeCompare(a));
                    
                    // Cari entry terakhir yang memiliki endPage valid
                    for (const date of dates) {
                        const dayProgress = progress[date];
                        if (dayProgress.endPage && dayProgress.endPage > 0) {
                            return dayProgress.endPage;
                        }
                    }
                ...
                const canSaveTahfizh = (santriId) => {
                    ...
                    // Jika ada input halaman, validasi harus benar
                    if (input.startPage || input.endPage) {
                        if (!input.startPage || !input.endPage) return false;
                        return parseInt(input.startPage) <= parseInt(input.endPage);
                    }
                    
                    // Jika tidak ada input halaman, tetap bisa simpan (untuk murojaah/ujian saja)
                    return true;
                };
                ...
                const progressData = {
                    date: today,
                    startPage: input.startPage ? parseInt(input.startPage) : null,
                    endPage: input.endPage ? parseInt(input.endPage) : null,
                    murojaahJuz: parseInt(input.murojaahJuz) || 0,
                    exam: input.exam ? input.exam.trim() : null,
                        ...
                        try {
                            const { error } = await supabaseClient.value
                                ...
                                .upsert({
                                    santri_id: santriId,
                                    date: today,
                                    start_page: progressData.startPage,
                                    end_page: progressData.endPage,
                                    murojaah_juz: progressData.murojaahJuz,
